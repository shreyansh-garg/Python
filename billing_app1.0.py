...
class BillingApp:
    ...
    def print_text_content(self, content, title="Print"):
        try:
            # Set page size based on report type
            if title.startswith("Detailed Sales Report"):
                page_width = 210 * mm  # A4 for Detailed Sales Report
                max_chars = 67
            else:
                page_width = 80 * mm  # 3-inch thermal paper
                max_chars = 42
            page_height = 297 * mm  # A4 height, adjustable
            buffer = BytesIO()
            c = canvas.Canvas(buffer, pagesize=(page_width, page_height))
            c.setFont("Courier", 10)

            # Split content into lines and draw on PDF
